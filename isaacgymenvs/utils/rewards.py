...
@torch.jit.script
def hand_pose_action_penalty(joint_pos, target_joint_pos, joint_deltas, joint_init, alpha: float = 0.5, beta: float = 1.):
    # combines 3 terms: control error (joint_pos - target_joint_pos), joint velocity (joint_deltas), and drift from default pos
    return alpha * torch.linalg.norm(joint_pos - target_joint_pos, dim=-1) + (1 - alpha) * torch.linalg.norm(joint_deltas, dim=-1) + beta * torch.linalg.norm(joint_pos - joint_init, dim=-1)
